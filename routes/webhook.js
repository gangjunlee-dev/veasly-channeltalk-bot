...
router.post('/channeltalk', async function(req, res) {
  try {
    ...
    var _quoteBlockers = ['破損','破损','解體','解体','壞掉','壞了','不會亮','打不開','卡住','沒有跳出','瑕疵','短少','少了','沒寄','漏寄',
      ...
      // 이미 견적받은 상품의 속성·상태 문의 — 見積 how-to 가 답이 아니다(중고 상태·구성·사이즈 등)
      '報價商品','此報價','這個報價','該報價',
      '髒污','清洗','下水','外出過','使用痕跡','狀態',
      '尺寸','顏色','材質','內容物','配件','贈品'];
      ...
      try {
        ...
        if (orderItems && orderItems.length > 0) {
          ...
          var tipMap = {
            ...
            "CANCEL_COMPLETED": { "zh-TW": "此訂單已取消！退款我們會在 3-5 個工作天內處理，實際退刷入帳依銀行/金流作業約需 7-14 個工作天喔！", "ko": "주문이 취소되었습니다! 환불은 저희가 3-5 영업일 내 처리하며, 실제 입금(退刷)은 PG/은행 작업에 따라 약 7-14 영업일 소요돼요!", "en": "Order cancelled! We process the refund within 3-5 business days; the actual refund to your card takes about 7-14 business days via the bank/PG.", "ja": "注文キャンセル済み！返金は3-5営業日以内に処理し、実際の返金（退刷）は銀行/決済会社により約7-14営業日かかります！" }
          };
          var tip = (tipMap[mainStatus] && tipMap[mainStatus][detectedLang]) || (tipMap[mainStatus] && tipMap[mainStatus]["zh-TW"]) || "";
          if (tip) orderReply += "\n\n📋 " + tip;
          // 주문 상태별 맞춤 후속 행동 제안
          // [2026-07-08] 신 9단계 followUp. 다단계 공유 객체. 취소 환불=처리 3-5일+실입금 7-14일(2단계 명확화). DELIVERED=7일 退換(구 COMPLETED에서 이동).
          var _fuCenter = { "zh-TW": "📦 商品已入物流中心，接著會檢查、打包並安排航班，請耐心等候喔！", "ko": "📦 상품이 물류센터에 입고됐어요. 이후 검사·포장·항공편 안배가 진행되니 조금만 기다려주세요!", "en": "📦 Your item is at our logistics center. Next: inspection, packing, and flight scheduling — please hold on!", "ja": "📦 商品が物流センターに入荷しました。この後、検査・梱包・フライト手配が進みます！" };
          var _fuProcessing = { "zh-TW": "📋 這階段會完成 EZ WAY 對應與打包。若尚未完成 EZ WAY 實名認證，請先至 App 完成，才能順利收到申報通知喔！", "ko": "📋 이 단계에서 EZ WAY 대응·포장이 진행돼요. EZ WAY 실명인증이 안 됐다면 App에서 먼저 완료해야 신고 통지를 받을 수 있어요!", "en": "📋 EZWAY handling and packing happen here. If you haven't done EZ WAY verification, complete it in the app to receive the declaration notice!", "ja": "📋 この段階でEZWAY対応・梱包を行います。EZ WAY実名認証がまだなら、Appで先に完了してください！" };
          ...
          var followUpMap = {
            ...
            "ARRIVED_AT_LOCAL": _fuCustoms, "CUSTOMS_HOLD": _fuCustoms,
            "DELIVERING": { "zh-TW": "🚚 配送途中恕無法變更地址，請留意收件；若顯示已送達卻沒收到，請先確認管理室或家人代收，再告訴我喔！", "ko": "🚚 배송 중에는 주소 변경이 어려워요. 수령에 유의해주시고, 배송완료 표시인데 못 받으셨다면 관리실/가족 대리수령 확인 후 알려주세요!", "en": "🚚 Address can't be changed during delivery. If it shows delivered but you didn't get it, please check with the front desk/family first, then let me know!", "ja": "🚚 配送中は住所変更ができません。配達済み表示なのに受け取っていない場合は、管理室やご家族の代理受取をご確認ください！" },
            "DELIVERED": { "zh-TW": "🎉 期待您的下次購物！如商品有問題，收到後 7 天內可申請退換貨喔～", "ko": "🎉 다음 쇼핑도 기대해주세요! 상품 문제 시 수령 후 7일 이내 교환/반품 가능합니다~", "en": "🎉 Hope you love it! If any issue, returns/exchanges are available within 7 days of receipt.", "ja": "🎉 またのご利用をお待ちしています！商品に問題があれば受取後7日以内に返品・交換できます～" },
            "COMPLETED": { "zh-TW": "🎉 訂單已完成，感謝您在 VEASLY 購物，期待下次為您服務！", "ko": "🎉 주문이 완료되었습니다. VEASLY를 이용해 주셔서 감사합니다!", "en": "🎉 Order complete. Thank you for shopping with VEASLY!", "ja": "🎉 注文完了です。VEASLYをご利用いただきありがとうございます！" },
            "CANCEL_COMPLETED": { "zh-TW": "💰 退款約 3-5 個工作天內處理，實際退刷入帳依銀行約 7-14 個工作天。如果想重新下單，可以直接在 veasly.com 申請報價喔！", "ko": "💰 환불은 3-5영업일 내 처리, 실제 입금은 약 7-14영업일 소요됩니다. 재주문을 원하시면 veasly.com에서 견적 요청해주세요!", "en": "💰 Refund processed in 3-5 business days; actual refund to your card in ~7-14 business days. Want to reorder? Visit veasly.com!", "ja": "💰 返金は3-5営業日以内に処理、実際の入金は約7-14営業日です。再注文はveasly.comからどうぞ！" }
          };
