...
async function analyzeRecentChats(days) {
  ...
  try {
    ...
    // === 키워드 동의어 그룹 ===
    var synonymGroups = {
      '訂單(주문)': ['訂單編號', '訂單號碼', '請問訂單號', '訂單號', '訂單', '订单', '單號'],
      '客服(상담사)': ['客服', '真人客服', '客服人員', '轉接客服', '人工客服'],
      '物流更新(배송현황)': ['物流沒有更新', '怎麼都沒有更新狀態', '怎麼都沒有更', '新狀態', '的進度', '物流更新', '沒有更新', '更新狀態'],
      '退款(환불)': ['退款', '退回', '退錢', '退費'],
      '運費(배송비)': ['運費', '國際運費', '運送費', '免運'],
      '到貨時間(배송기간)': ['到貨', '幾天', '多久', '來得及', '期了', '什麼時候到'],
      '處理進度(처리현황)': ['處理中', '處理進度', '幫忙確認', '可以幫忙', '確認一下'],
      '網站(웹사이트)': ['網站', '網頁', '官網']
    };
    var synonymMap = {};
