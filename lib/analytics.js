...
var CATEGORIES = {
  ...
  shipping: {
    name: '配送相關',
    nameKo: '배송/물류',
    keywords: ['배송', '物流', '配送', '出貨', '發貨', '寄出', '到貨', '快遞', '順豐', '택배', '도착', '집운', '集運', '寄', '合併', '併單', '합배송', '香港', '寄到', '貨運', '包裹', '多久', '還要多久', '一個禮拜', '禮拜'],
    priority: 3
  },
  shipping_fee: {
    name: '國際運費',
  ...
  complaint: {
    name: '客訴/不滿',
    nameKo: '클레임/불만',
    keywords: ['投訴', '差評', '很爛', '太慢', '太久', '拖了', '별로', '왜', '為什麼', '不滿', '失望', '多收', '被騙', '誇張', '離譜', '生氣', '처리', '問題', '沒有提到', '不知道', '完全沒有', '回覆', '不智能', '傻眼', '混亂', '多收錢', '沒有處理', '不合理', '受害者', '沒有主動', '不太對'],
    priority: 10.5
  },
  greeting: {
    name: '問候/感謝',
...
async function analyzeRecentChats(days) {
  ...
  try {
    ...
    for (var i = 0; i < allChats.length; i++) {
      ...
      var _hadRecentMsg = false; var _hadUserMsg = false;
      ...
      try {
        ...
        for (var m = 0; m < messages.length; m++) {
          ...
          if (msg.personType === 'user') {
            results.userMessages++; _hadUserMsg = true;
        ...
        if (_hadUserMsg) results.totalChats++;
      } catch (msgErr) {
