  ...
  try {
    ...
    for (var i = 0; i < chats.length; i++) {
      ...
      if (bizHoursPassed >= 16 && !isLine) {
        ...
        if (!csatHelper.alreadySent(chat.id)) { csatHelper.markWarning(chat.id); csatSentData = csatHelper.load(); }

      } else if (bizHoursPassed >= 4) {
        if (!csatHelper.alreadySent(chat.id)) { csatList.push({ id: chat.id, hours: Math.floor(hoursPassed), bizHours: Math.floor(bizHoursPassed), userId: chat.userId }); }
      }
    ...
    for (var c = 0; c < csatList.length; c++) {
      try {
        ...
        csatSentData = csatHelper.load(); // refresh via csat module
        // Check csat-sent.json
        if (csatHelper.alreadySent(csatChatId)) { console.log("[Scheduler] CSAT already sent to:", csatChatId, "- skip"); continue; }
                // Check csat-results.json (already responded)
                var _csatResults = {};
                try { _csatResults = JSON.parse(fs.readFileSync(require("path").join(__dirname, "..", "data", "csat-results.json"), "utf8")); } catch(_re) {}
                if (_csatResults[csatChatId]) { console.log("[Scheduler] CSAT already responded:", csatChatId, "- skip"); continue; }
        ...
        // 매니저가 실제 응답했는지 확인 (봇만 응답한 경우 CSAT 미발송)
        try {
          var _msgCheck = await channeltalk.getChatMessages(csatChatId, 50);
          var _hasManagerReply = (_msgCheck.messages || []).some(function(m) {
            return m.personType === "manager" && m.personId && m.personId !== "0";
          });
          if (!_hasManagerReply) {
            console.log("[Scheduler] CSAT skip - no manager reply:", csatChatId);
            csatHelper.markSkipped(csatChatId, "no-manager-reply");
            continue;
          }
        } catch(_chkErr) { console.log("[Scheduler] Manager check error:", _chkErr.message); }

        await channeltalk.sendMessage(csatChatId, { blocks: [{ type: "text", value: csatMsg }] });
        csatHelper.markSent(csatChatId, "scheduler");
...
function isCSATPending(chatId) {
  ...
  if (sent[chatId]) {
    // responded 문자열이면 이미 응답 완료 → pending 아님
    if (typeof sent[chatId] === "string" && sent[chatId].indexOf("responded") === 0) return false;
    // responded 객체면 이미 응답 완료 → pending 아님
    if (typeof sent[chatId] === "object" && sent[chatId] !== null && sent[chatId].responded) return false;
    return true;
  }
  // csat-results.json에 이미 있으면 응답 완료
  try {
    var results = JSON.parse(fs.readFileSync(require("path").join(__dirname, "..", "data", "csat-results.json"), "utf8"));
    if (results[chatId]) return false;
  } catch(e) {}
  return false;
}
