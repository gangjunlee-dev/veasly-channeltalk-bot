...
function startScheduler() {
  ...
  cron.schedule('0 10 * * 1-5', async () => {
    ...
    try {
      var chats = await channeltalk.getChats({ state: 'opened', limit: 100 });
      var openChats = (chats && chats.chats) || [];
      var urgentCount = 0;
      var urgentList = [];
      
      openChats.forEach(function(chat) {
        if (!chat.lastMessage) return;
        var lastMsgTime = chat.lastMessage.createdAt || 0;
      ...
      if (urgentCount > 0) {
        ...
        try {
          ...
          if (groups && groups.length > 0) {
            await channeltalk.sendGroupMessage(groups[0].id, alertMsg);
          }
  ...
  cron.schedule('0 15 * * 1-5', async () => {
    ...
    try {
      ...
      var longWait = openChats.filter(function(chat) {
        ...
        var hoursSince = (Date.now() - (chat.lastMessage.createdAt || 0)) / (1000 * 60 * 60);
        return hoursSince >= 2;
      });
