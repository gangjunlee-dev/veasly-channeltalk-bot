  ...
  try {
    ...
    for (var i = 0; i < chats.length; i++) {
      ...
      if (bizHoursPassed >= 16 && !isLine) {
        ...
        if (!csatSentData[chat.id]) { csatList.push({ id: chat.id, hours: Math.floor(hoursPassed), bizHours: Math.floor(bizHoursPassed), userId: chat.userId }); }
      }
    ...
    for (var c = 0; c < csatList.length; c++) {
      try {
        ...
        csatSentData = loadCSATSent(); // refresh
        ...
        try {
          ...
          if (!_hasManagerReply) {
            ...
            csatSentData[csatChatId] = { sentAt: Date.now(), count: 0, skipped: true };
            saveCSATSent(csatSentData);
            continue;
          }
