...
async function handoffTriage() {
  ...
  var resolved = 0, flagged = 0, seenChat = {};
  for (var i = 0; i < rows.length; i++) {
    ...
    if (verdict.v === 'resolve') {
      try {
        await axios.patch('https://api.notion.com/v1/pages/' + p.id, { properties: { '상태': { select: { name: '해결' } } } }, NH);
        resolved++;
      } catch (e) {}
      await zzz(300);
    }
  }
  console.log('[HandoffTriage] 해결 처리:', resolved, '| 미해결 플래그:', flagged, '| 검사 상담:', Object.keys(seenChat).length);
  if (resolved || flagged) {
    try { await notifyInternal('🔄 넘김 데일리 정리: 실질 마무리 ' + resolved + '건 자동 해결' + (flagged ? ' / 미해결·약속 미이행 ' + flagged + '건 → 약속 추적(오늘 독촉 목록 포함)' : '')); } catch (e) {}
  }
...
function checkPendingEscalations() {
  ...
  cron.schedule('*/10 * * * *', async () => {
    ...
    // [2026-08-24] 통합 플랫폼(veasly-ops) 대시보드로 스냅샷 전송 — 스윕 직후라 데이터가 최신
    try { await require('./ops-push').pushSnapshot(); } catch (e) { console.error('[Scheduler] OpsPush error:', e.message); }
  });
  ...
  cron.schedule('0 9 * * 1-5', async () => {
    ...
    try { await require('./ops-push').pushSnapshot({ withHistory: true }); } catch (e) {}
  });
