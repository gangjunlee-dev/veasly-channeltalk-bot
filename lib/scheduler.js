  ...
  try {
    ...
    for (var w = 0; w < warningList.length; w++) {
      try {
        ...
        // 유저 정보 조회 (회원 여부, 이메일, VEASLY ID)
        var _wUserInfo = { member: false, email: '', veaslyId: '' };
        try {
        ...
        var _wSurveyUrl = _wBaseUrl + "?cid=" + warnChatId + "&uid=" + (warningList[w].userId || "") + "&lang=" + warnLang + "&type=bot&ts=" + Math.floor(Date.now()/1000) + "&member=" + (_wUserInfo.member ? "1" : "0") + "&email=" + encodeURIComponent(_wUserInfo.email) + "&vid=" + encodeURIComponent(_wUserInfo.veaslyId);
        var warnMsgs = {
    ...
    for (var d = 0; d < closeList.length; d++) {
      try {
        ...
        var recentMsgs = [];
        try { var _resolveMsgs = await channeltalk.getChatMessages(closeChatId, 20); recentMsgs = (_resolveMsgs.messages || []).map(function(m){ return { role: m.personType === "manager" ? "bot" : "user", text: (m.blocks && m.blocks[0] && m.blocks[0].value) || "" }; }); } catch(_rmErr) {}
