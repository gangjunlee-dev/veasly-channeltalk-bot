  ...
  try {
    ...
    for (var i = 0; i < chats.length; i++) {
      ...
      if (bizHoursPassed >= 16 && !isLine) {
        ...
        // REMOVED: CSAT warning 마킹 (자동종료 시 발송으로 변경)
    ...
    for (var d = 0; d < closeList.length; d++) {
      try {
        ...
        // 종료 메시지 + CSAT를 함께 발송
        var _closeLang = chatLangs[closeChatId] || "zh-TW";
        var closeMsgs = {
          "zh-TW": "此對話因長時間無新訊息，將自動結束。如有其他問題，歡迎隨時開啟新對話！😊\n\n📋 最後想請問，這次的服務體驗如何呢？\n1️⃣ 非常滿意  2️⃣ 滿意  3️⃣ 普通  4️⃣ 不滿意  5️⃣ 非常不滿意\n請輸入數字 1~5",
          "ko": "이 상담은 장시간 추가 메시지가 없어 자동 종료됩니다. 추가 문의가 있으시면 언제든 새 채팅을 시작해주세요! 😊\n\n📋 마지막으로, 이번 서비스는 어떠셨나요?\n1️⃣ 매우 만족  2️⃣ 만족  3️⃣ 보통  4️⃣ 불만족  5️⃣ 매우 불만족\n숫자 1~5를 입력해주세요",
          "en": "This chat has been automatically closed due to inactivity. Feel free to start a new chat anytime! 😊\n\n📋 How was your experience?\n1️⃣ Very Satisfied  2️⃣ Satisfied  3️⃣ Neutral  4️⃣ Dissatisfied  5️⃣ Very Dissatisfied\nPlease enter 1~5",
          "ja": "長時間メッセージがないため、このチャットを自動終了します。ご質問があればいつでも新しいチャットを開始してください！😊\n\n📋 今回のサービスはいかがでしたか？\n1️⃣ 大満足  2️⃣ 満足  3️⃣ 普通  4️⃣ 不満  5️⃣ 大不満\n1~5の数字を入力してください"
        };
        var closeMsg = closeMsgs[_closeLang] || closeMsgs["zh-TW"];

        await channeltalk.sendMessage(closeChatId, { blocks: [{ type: "text", value: closeMsg }] });
        csatHelper.markSent(closeChatId, "auto-close");
        console.log("[Scheduler] CSAT sent with close msg:", closeChatId);
    ...
    if (unresolvedList.length > 0) {
      console.log("[Scheduler] Unresolved check done:", unresolvedList.length, "overdue chats (" + csatSentCount + " CSAT at close,", closedCount, "closed)");
