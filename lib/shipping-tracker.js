...
// [2026-08-18 제거] ORDER_PROCESSING 능동 알림 삭제 — 다시 넣지 말 것.
//   이 단계는 "한국에서 구매를 진행 중"일 뿐 판매자가 주문을 수락했다는 뜻이 아니다
//   (STATUS_MAP: ORDER_PROCESSING = 商品準備中). 그런데 문구가 「賣家正在準備出貨」로 단정해
//   閃電拍賣(BUNJANG 개인 중고) 건에서 실제 사고가 났다:
//   8/5 결제 → 8/6 이 알림 발송 → 고객이 '구매 확정'으로 이해 → 8/12 商品已售出 자동취소
//   → "客服가 샀다고 했는데 거짓말이었다"는 불만·보상 요구(주문 20260805TW190832623).
//   아래 나머지 알림은 전부 물건이 실제로 VEASLY 손에 들어온 뒤의 사실이라 유지한다.
//   ※ 상태 추적(newState) 자체는 계속되므로 다음 단계 전이 알림은 정상 발송된다.
