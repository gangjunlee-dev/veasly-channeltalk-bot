...
// [2026-08-24] CS 넘김 DB 상태 분포 (대시보드용)
async function handoffStatusCounts() {
  if (!NOTION_TOKEN) return null;
  try {
    var counts = {}, cursor;
    do {
      var b = { page_size: 100 }; if (cursor) b.start_cursor = cursor;
      var r = await axios.post('https://api.notion.com/v1/databases/' + DB_ID + '/query', b, _nh());
      (r.data.results || []).forEach(function (p) {
        var s = '(없음)'; try { s = p.properties['상태'].select.name; } catch (e) {}
        counts[s] = (counts[s] || 0) + 1;
      });
      cursor = r.data.has_more ? r.data.next_cursor : null;
    } while (cursor);
    return counts;
  } catch (e) { console.error('[Notion] handoffStatusCounts 실패:', e.message); return null; }
}

module.exports = {
  createHandoffEntry: createHandoffEntry,
  handoffStatusCounts: handoffStatusCounts,
