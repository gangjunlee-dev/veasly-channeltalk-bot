...
async function getOrderDetail(orderNumber) {
  ...
  try {
    var res = await client.get("/admin/orders/" + orderNumber + "/detail");
    return res.data;
  } catch(e) { console.error("[VEASLY API] getOrderDetail error:", e.message); return null; }
}

async function getUserOrders(userEmail, limit) {
  limit = limit || 500;
  if (!userEmail) return [];
  try {
    var res = await client.get("/admin/orders/0/" + limit);
    var orders = (res.data && res.data.data) || [];
    return orders.filter(function(o) {
      return o.user && o.user.email && o.user.email.toLowerCase() === userEmail.toLowerCase();
    });
