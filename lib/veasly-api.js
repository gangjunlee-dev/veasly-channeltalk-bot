...
async function getUserOrders(userEmail, limit, userId) {
  try {
    ...
    var orders = (res.data && res.data.data) || [];
    // SECURITY: the admin orders API IGNORES the USER_EMAIL filter and returns a
    // global recent-orders feed. We MUST filter client-side to this account,
    // otherwise other customers' orders leak into the reply (see incident 2026-06-01).
    // Match strictly by owner id; fall back to exact email only when no userId given.
