...
async function findUserById(memberId, email) {
  try {
    ...
    // [2026-08-18 FIX] 이메일이 없으면 회원 매칭이 사실상 실패하고 있었다.
    //   아래 자유 검색(queryType 없음)은 memberId 와 무관한 50건을 받아와 id 일치가 거의 안 났다(=죽은 폴백).
    //   queryType=USER_ID 로 직접 조회하면 이메일 없이도 정확히 1건이 나온다.
    //   실측(2026-08-18, 최근 상담 80건): 식별률 57% → 78% (이메일 없는 17건 중 16건 구제).
    //   ※ memberId 가 UUID 형이면 채널톡 내부 ID(=VEASLY 회원 아님)라 조회 대상이 아니므로 건너뛴다.
    if (memberId && /^\d+$/.test(String(memberId))) {
      var resId = await apiGet("/admin/users/0/50?query=" + encodeURIComponent(String(memberId)) + "&queryType=USER_ID");
      var usersId = (resId.data && resId.data.data) || [];
      var matchId = usersId.find(function(u) { return String(u.id) === String(memberId); });
      if (matchId) return matchId;
    }
    // If no email or no match, search all and filter (마지막 폴백 — 기존 동작 유지)
