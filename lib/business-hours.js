...
var KR_HOLIDAYS_2026 = [
  ...
  '2026-06-03', // 지방선거일
  '2026-06-06', // 현충일
  '2026-07-17', // 제헌절 (2026년 공휴일 재지정 — CS 휴무)
  '2026-08-15', // 광복절
...
var TW_HOLIDAYS_2026 = [
  '2026-01-01', // 元旦
  '2026-02-14', '2026-02-15', '2026-02-16', '2026-02-17', '2026-02-18', '2026-02-19', '2026-02-20', '2026-02-21', '2026-02-22', // 春節 (含彈性)
  '2026-02-27', // 和平紀念日連假 (2/28 週六提前)
  '2026-02-28', // 和平紀念日
  '2026-04-03', '2026-04-04', '2026-04-05', '2026-04-06', // 兒童節+清明連假
  '2026-05-01', // 勞動節
  '2026-06-19', // 端午節 (原5/31錯誤已修正)
  '2026-09-25', // 中秋節 (原10/4-5錯誤已修正)
  '2026-09-28', // 教師節
  '2026-10-09', // 國慶日連假 (10/10 週六提前)
  '2026-10-10', // 國慶日
  '2026-10-26', // 台灣光復節 大替休日
  '2026-12-25'  // 行憲紀念日
];
...
/**
 * 특정 Date 객체 기준으로 영업시간인지 판단
 * @param {Date} date - 판단할 시점 (UTC)
 */
function isBusinessHoursAt(date) {
  var kst = new Date(date.getTime() + 9 * 60 * 60 * 1000);
  var day = kst.getUTCDay();
  var hour = kst.getUTCHours();
  var dateStr = getKSTDateString(kst);
  
  if (day === 0 || day === 6) return false;
  if (KR_HOLIDAYS_2026.indexOf(dateStr) !== -1) return false;
  if (TW_HOLIDAYS_2026.indexOf(dateStr) !== -1) return false;
  return hour >= 10 && hour < 19;
}


// 2026년 공휴일명 매핑 (공휴일 안내 메시지용)
var KR_HOLIDAY_NAMES = {
...
var TW_HOLIDAY_NAMES = {
  ...
  '2026-02-27': '和平紀念日', '2026-02-28': '和平紀念日',
  '2026-04-03': '兒童節', '2026-04-04': '兒童節', '2026-04-05': '清明節', '2026-04-06': '清明節',
  '2026-05-01': '勞動節', '2026-05-24': '佛誕節', '2026-05-25': '佛誕節補假', '2026-06-19': '端午節',
  '2026-07-17': '韓國制憲節',
  '2026-09-25': '中秋節', '2026-09-28': '教師節',
  '2026-10-09': '國慶日', '2026-10-10': '國慶日', '2026-10-26': '台灣光復節',
  '2026-12-25': '行憲紀念日'
};

// [2026-08-14] 한국 공휴일의 중문(台灣華語) 표기.
// 기존엔 TW_HOLIDAY_NAMES(대만 공휴일)에서만 찾아, 한국 고유 공휴일(광복절·현충일·추석 등)은
// twName=null → 고객 안내문에 한글 이름이 그대로 노출되던 문제를 수정.
var KR_HOLIDAY_NAMES_ZH = {
  '2026-01-01': '元旦',
  '2026-02-16': '韓國春節', '2026-02-17': '韓國春節', '2026-02-18': '韓國春節',
  '2026-03-01': '韓國三一節', '2026-03-02': '韓國三一節補假',
  '2026-05-01': '勞動節', '2026-05-05': '韓國兒童節',
  '2026-05-24': '佛誕節', '2026-05-25': '佛誕節補假',
  '2026-06-03': '韓國地方選舉日', '2026-06-06': '韓國顯忠日',
  '2026-07-17': '韓國制憲節',
  '2026-08-15': '韓國光復節', '2026-08-17': '韓國光復節補假',
  '2026-09-24': '韓國中秋節', '2026-09-25': '韓國中秋節', '2026-09-26': '韓國中秋節',
  '2026-10-03': '韓國開天節', '2026-10-05': '韓國開天節補假',
  '2026-10-09': '韓國韓文節',
  '2026-12-25': '聖誕節'
};
...
/**
 * 현재 공휴일명 반환 (KR/TW)
 * twName은 한국 공휴일 중문표기 우선(고객 안내문에 사용), 없으면 대만 공휴일명 폴백.
 * @returns {{ isHoliday: boolean, krName: string|null, twName: string|null }}
 */
function getHolidayInfo(timestamp) {
  var dateStr = getKSTDateString(timestamp);
  return {
    isHoliday: KR_HOLIDAYS_2026.indexOf(dateStr) >= 0,
    krName: KR_HOLIDAY_NAMES[dateStr] || null,
    twName: KR_HOLIDAY_NAMES_ZH[dateStr] || TW_HOLIDAY_NAMES[dateStr] || null
  };
}

module.exports = {
  getHolidayInfo: getHolidayInfo,
  KR_HOLIDAY_NAMES: KR_HOLIDAY_NAMES,
  TW_HOLIDAY_NAMES: TW_HOLIDAY_NAMES,
  isBusinessHoursAt: isBusinessHoursAt,
