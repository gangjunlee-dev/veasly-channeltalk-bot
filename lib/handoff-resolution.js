...
async function generateResolution(msgs, meta) {
  ...
  var prompt =
    ...
    'RULE: <한 줄>';
  try {
    // [2026-08-24] maxTokens 400 → 2000. 모델이 추론에 토큰을 다 쓰면 text 블록이 비어
    //   '근거 부족'으로 오판된다(llm.js 는 type==='text' 만 추출). 실제로 sopCandidates 가
    //   stop_reason=max_tokens + 빈 텍스트로 실패했다.
    var res = await llm.generate({ user: prompt, maxTokens: 2000 });
    var raw = (res && res.text) || '';
    if (!raw) {
      if (res && res.stopReason === 'max_tokens') console.warn('[HandoffResolution] 토큰 부족으로 빈 응답(근거부족 아님)');
      return '';
    }
    var result = '', rule = '';
...
async function sopCandidates() {
  if (!process.env.NOTION_TOKEN || !llm.isEnabled()) return null;
  var all = await _queryAll();
  // 사유별로 미리 묶는다(코드) → 사유마다 작은 LLM 호출(의미 클러스터링).
  //   [2026-08-24] 150건을 한 번에 주면 모델이 추론에 4000토큰 이상을 써서 text 가 비어 실패했다.
  //   사유별 소규모 호출은 3초 내 안정적으로 응답한다.
  var byReason = {};
  all.forEach(function (p) {
    var v = _rt(p, '처리 결과 / 한 줄 규칙');
    var m = v.match(/규칙:\s*(.+)$/);
    if (!m) return;
    var r = _sel(p, '넘김 사유') || '기타';
    if (!byReason[r]) byReason[r] = [];
    byReason[r].push(m[1].trim());
  });
  var total = Object.keys(byReason).reduce(function (a, k) { return a + byReason[k].length; }, 0);
  if (total < 5) return { total: total, text: null };
  var NL2 = String.fromCharCode(10);
  var sections = [];
  var reasons = Object.keys(byReason).sort(function (a, b) { return byReason[b].length - byReason[a].length; });
  for (var i = 0; i < reasons.length; i++) {
    var rs = byReason[reasons[i]];
    if (rs.length < 3) continue; // 3건 미만 사유는 승격 대상 아님
    var prompt = [
      '아래는 CS 넘김 기록에서 뽑은 「직원 처리 가능 범위」 규칙들이다(사유: ' + reasons[i] + ').',
      '의미가 같은 것끼리 묶어, 3건 이상 반복되는 주제만 최대 3개 뽑아라.',
      '각 주제는 정확히 2줄: 「· <주제>(N건)」 과 「  규칙: <SOP 에 넣을 한 줄>」.',
      '3건 미만 주제는 버린다. 목록에 없는 내용은 만들지 않는다. 다른 설명 금지.',
      '',
      rs.slice(-25).join(NL2)
    ].join(NL2);
    try {
      // 이 모델은 클러스터링에서 추론 토큰을 많이 쓴다(40건·1500 에서도 부족). 실제 출력은 200토큰 이하라 여유만 크게 준다.
      var res = await llm.generate({ user: prompt, maxTokens: 8000, timeoutMs: 150000 });
      var txt = (res && res.text) ? res.text.trim() : '';
      if (!txt && res && res.stopReason === 'max_tokens') console.warn('[SOPCandidates] 토큰 부족:', reasons[i]);
      if (txt) sections.push('▸ ' + reasons[i] + ' (누적 ' + rs.length + '건)' + NL2 + txt);
    } catch (e) { console.error('[SOPCandidates] ' + reasons[i] + ' 오류:', e.message); }
  }
  return { total: total, text: sections.length ? sections.join(NL2 + NL2) : null };
}
