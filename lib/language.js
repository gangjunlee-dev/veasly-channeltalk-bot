function detectLanguage(text) {
  ...
  for (var i = 0; i < text.length; i++) {
    ...
    else if ((code >= 0x41 && code <= 0x5A) || (code >= 0x61 && code <= 0x7A)) en++; // [2026-06-29] 알파벳만(기존엔 [\]^_` 기호 6개도 오집계)
  }
  var max = Math.max(ko, zh, en, ja);
  if (max === 0) return 'zh-TW';
  // [2026-06-29] かな(kana)는 일본어 전용 → 한자가 더 많아도 일본어로 확정.
  // (한자 위주 일본어가 zh-TW로 오인돼 일본 손님이 중국어 답을 받던 버그 수정)
  if (ja > 0) return 'ja';
  // 한글이 중국어보다 '확실히' 우세할 때만 ko (대만 손님이 한글 브랜드명을 섞어도 번체 중국어 유지)
  if (ko > 0 && ko > zh) return 'ko';
  if (zh > 0) return 'zh-TW';
  if (en > 0) return 'en';
  return 'zh-TW';
}
