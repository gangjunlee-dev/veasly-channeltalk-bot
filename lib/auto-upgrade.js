...
// [2026-07-06] Gemini/Pinecone 폐기 — FAQ 자동생성 기능 비활성(노션 단일소스). require 제거.
...
// [2026-06-30] Gemini가 답변 문자열 값 안에 이스케이프 안 된 raw 개행/탭/제어문자를 넣어 보내면
// JSON.parse가 "Bad control character in string literal"로 거부함. 문자열 리터럴 '내부'의 제어문자만
// 유효한 이스케이프로 바꾸고, 토큰 사이의 구조적 공백(들여쓴 개행 등)은 건드리지 않는다(그건 합법이라 보존).
...
async function processEscalationCandidates() {
  // [2026-07-06] 비활성: Gemini FAQ 생성 → Pinecone 적재는 노션 전환으로 폐기됨. (아래 원본 로직은 실행되지 않음)
  return { processed: 0, added: 0, disabled: 'notion 전환' };
  ...
  for (var category in targetGroups) {
    ...
    try {
      ...
      var faqs = JSON.parse(sanitizeJsonControlChars(jsonMatch[0]));
      ...
      for (var fi = 0; fi < faqs.length; fi++) {
        ...
        await aiEngine.addToKnowledgeBase(faqId, faqText, {
          namespace: "faq_review", // [2026-06-29] AI 추측 FAQ는 라이브 KB 직행 금지 → 사람 검토 큐(faq_review)로
          pending_review: true,
...
async function processReviewFeedback() {
  // [2026-07-06] 비활성: Gemini 수정FAQ → Pinecone 적재는 노션 전환으로 폐기됨. (아래 원본 로직은 실행되지 않음)
  return { processed: 0, fixed: 0, disabled: 'notion 전환' };
  ...
  for (var ri = 0; ri < pending.length; ri++) {
    ...
    try {
      var result = await model.generateContent(prompt);
      var text = result.response.text().replace(/```json?\n?/g, "").replace(/```/g, "").trim();
      var faq = JSON.parse(sanitizeJsonControlChars(text));

      var faqText = "Q: " + faq.question + "\nA: " + faq.answer;
      var faqId = "review_fix_" + Date.now() + "_" + ri;

      await aiEngine.addToKnowledgeBase(faqId, faqText, {
        namespace: "faq_review", // [2026-06-29] 검토 후 승격 (라이브 KB 직행 금지)
        pending_review: true,
