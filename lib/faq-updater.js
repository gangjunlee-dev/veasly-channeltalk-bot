...
async function updateFAQNamespace(faqs) {
  ...
  for (var i = 0; i < faqs.length; i++) {
    try {
      ...
      // 검증 안 된 매니저 응답 요약은 조회되는 'faq' 네임스페이스에 넣지 않는다.
      // (이게 답변 KB를 오염시켜 잘못된 답을 유발했음 — 2026-06-01 검토)
      // 사람이 검토 후 승격하도록 조회되지 않는 'faq_review' 네임스페이스에 보관.
      await aiEngine.addToKnowledgeBase(faqId, faqText, {
        namespace: "faq_review",
...
async function cleanupOldManagerEntries(entries, keepDays) {
  ...
  try {
    ...
    for (var i = 0; i < ids.length; i += 100) {
      ...
      await index.namespace("manager").deleteMany({ ids: batch });
    }
...
async function runFAQUpdate() {
  ...
  if (pendingReviews.length > 0) {
    ...
    for (var ci = 0; ci < corrections.length; ci++) {
      ...
      await aiEngine.addToKnowledgeBase(corrId, corrText, { namespace: "faq_review", pending_review: true, source: "admin_review_fix", category: corr.category || "general", timestamp: new Date().toISOString() }); // [2026-06-29] 라이브 KB 직행 금지 → 검토 큐
