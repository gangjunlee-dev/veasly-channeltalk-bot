 ...
 */
async function classifyAwaiting(prefetched) {
  var all = prefetched || await pageOpened();
...
/**
 * 묵은 미회신 목록(24영업시간+) — 아침 독촉에 붙일 섹션.
 * 래더는 당일 건만 다루므로(도배 방지), 이월된 건은 여기서 매일 아침 한 번 올린다.
 */
async function oldBacklogText(maxItems) {
  var res = await classifyAwaiting();
  var old = res.real.filter(function (r) { return r.bizH >= 24; });
  if (!old.length) return null;
  var lines = old.slice(0, maxItems || 10).map(function (r) {
    return '• [' + Math.round(r.bizH / 8) + '영업일 경과] ' + r.name.slice(0, 16) + ' — 「' + r.text.slice(0, 45) + '」\n  ' + r.link;
  });
  return '📮 이월 미회신 ' + old.length + '건 (고객이 답을 기다리는 중)\n' + lines.join('\n') +
    (old.length > (maxItems || 10) ? '\n…외 ' + (old.length - (maxItems || 10)) + '건' : '');
}

/**
 * 대시보드용 스냅샷 — 상담 목록을 한 번만 페이징해 미회신·지표·약속·넘김 현황을 모아 반환.
 * veasly-ops 통합 플랫폼(/cs-sla)이 이 JSON을 그대로 렌더한다.
 */
async function buildSnapshot() {
  var all = await pageOpened();
  var cls = await classifyAwaiting(all);
  var now = Date.now();

  // 첫 사람응답 소요(h) 분포 — 채널톡이 계산해주는 firstRepliedAt 활용
  var fr = all.filter(function (c) { return c.firstRepliedAt && c.createdAt; })
    .map(function (c) { return (c.firstRepliedAt - c.createdAt) / 3600000; })
    .sort(function (a, b) { return a - b; });
  function pct(arr, p) { return arr.length ? Math.round(arr[Math.min(arr.length - 1, Math.floor(arr.length * p))] * 10) / 10 : null; }

  var real = cls.real.map(function (r) {
    return { id: r.id, name: r.name, text: r.text, bizH: Math.round(r.bizH * 10) / 10, link: r.link, assigned: !!r.assigneeId };
  });
  ...
  try {
    ...
    promises = (await notionLib.listOverduePromises()).map(function (p) {
      return {
        customer: p.customer || p.title || '', due: p.due, reinquiry: p.reinquiry || 0,
        overdueDays: p.due ? Math.max(0, Math.floor((now - new Date(p.due).getTime()) / 86400000)) : 0,
        chatLink: p.chatLink || '', pageUrl: p.pageUrl || ''
      };
    });
  } catch (e) { console.error('[Snapshot] promises 실패:', e.message); }

  // CS 넘김 상태 분포(노션)
  var handoff = null;
  try { handoff = await require('./notion').handoffStatusCounts(); } catch (e) {}

  return {
    generatedAt: new Date().toISOString(),
    openedTotal: all.length,
    awaiting: {
      real: real.length, ghosts: cls.ghosts.length,
      today: real.filter(function (r) { return r.bizH < 8; }).length,
      carried: real.filter(function (r) { return r.bizH >= 8 && r.bizH < 24; }).length,
      stale: real.filter(function (r) { return r.bizH >= 24; }).length,
      items: real.slice(0, 60)
    },
    metrics: {
      firstReplyMedianH: pct(fr, 0.5), firstReplyP90H: pct(fr, 0.9),
      neverReplied: all.filter(function (c) { return !c.firstRepliedAt; }).length,
      sampleSize: fr.length
    },
    promises: { overdue: promises.length, items: promises.slice(0, 40) },
    handoff: handoff
  };
}
