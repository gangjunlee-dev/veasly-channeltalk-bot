...
// 생성된 답변이 참고자료/정책으로 뒷받침되는지 LLM으로 검증. 호출 실패 시 fail-open(true).
async function validateAnswer(answer, referenceText) {
  try {
    ...
    console.error('[AI][validation] error (fail-open):', e.message);
    return true;
  }
}
  ...
  try {
    ...
    if (isShippingQ && answer) {
      ...
      if (answer.includes('165') || answer.includes('首0.5') || answer.includes('first 0.5') || answer.includes('첫 0.5') || (answer.includes('0.5') && answer.includes('公斤'))) {
        ...
        answer = '國際運費是每公斤TWD 310起算喔！以實際重量和材積重量較高的那個為準，沒有0.5公斤的計費方式。寄出後大約7~14天會到，不會額外加收費用的～';
      }
      // [removed] 310 보충 로직 제거 - temperature 0.15 + 프롬프트로 충분
    }
    ...
    return { answer: answer.trim(), confidence: topScore, grounded: grounded };
    }
    return null;
  } catch (err) {
