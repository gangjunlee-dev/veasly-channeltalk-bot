...
async function generateAnswer(userMessage, language, chatId) {
  if (!AI_ENABLED) return null;
  try {
    var queryVector = await getQueryEmbedding(userMessage);
    var faqResult = await pineconeIndex.namespace("faq").query({ vector: queryVector, topK: 3, includeMetadata: true });
    var mgrResult = await pineconeIndex.namespace("manager").query({ vector: queryVector, topK: 2, includeMetadata: true }).catch(function(){ return {matches:[]}; });
    var siteResult = await pineconeIndex.namespace("site").query({ vector: queryVector, topK: 2, includeMetadata: true }).catch(function(){ return {matches:[]}; });
    ...
    if (answer && answer.trim().length > 0) {
      console.log('[AI] Generated answer for chatId:', chatId);
      return answer.trim();
    }
    return null;
  } catch (err) {
