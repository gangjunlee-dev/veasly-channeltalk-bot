 ...
 */
// 짝 없는 유니코드 서로게이트(깨진 이모지 등) 제거 → API "no low surrogate in string" 400 방지.
// Buffer UTF-8 왕복이 짝 없는 서로게이트만 U+FFFD로 치환, 정상 문자·이모지는 보존.
function cleanStr(s) { return Buffer.from(String(s == null ? '' : s), 'utf8').toString('utf8'); }
...
async function generate(o) {
  ...
  if (o.systemStable) {
    ...
    system.push({ type: 'text', text: cleanStr(o.systemStable), cache_control: { type: 'ephemeral' } });
  }
  if (o.systemVolatile) {
    system.push({ type: 'text', text: cleanStr(o.systemVolatile) });
  }
  var body = {
    model: o.model || MODEL,
    max_tokens: o.maxTokens || 1024,
    messages: [{ role: 'user', content: cleanStr(o.user) }]
  };
  ...
  for (var attempt = 0; attempt < 3; attempt++) {
    try {
      var res = await axios.post(API_URL, body, {
        ...
        timeout: o.timeoutMs || 30000 // [2026-08-24] 옵션화 — 주간 SOP 클러스터링처럼 긴 생성은 30s 로 부족
      });
      var d = res.data || {};
